...
class ConstraintSet(object):
    ...
    def score(self, individual: Individual) -> t.Tuple[float, ...]:
        unweighted_values = tuple(constraint.score(individual) for constraint in self._constraints)

        return (
            functools.reduce(
                operator.mul, (value**weight for value, weight in zip(unweighted_values, self._weights))
            ),
        ) + unweighted_values
